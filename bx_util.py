...
def estimate_q(X, opts=default_opts, bx=None):
    ...
    if bx is not None:
        if ds:
            m0_1 = compute_m0(bx, h)
            m0_2 = compute_m0(np.sqrt(s)*bx, h)
            m0 = ((m0_1**(-s))*(m0_2))**(1/(1-s))
        else:
            m0 = compute_m0(bx, h)
    else:
        m0 = 1
    ...
    n = X.shape[0]
    if ds:
        Z = ((np.pi)**(d/2))*(h**d)*(s**(d/(2*(s-1))))
        q = (np.array((K.power(s)).sum(axis=1)).flatten())**(1/(1-s))/(n-1)
        q = q/(Z*m0)
    else:
        q =  (np.pi**(-d/2))*np.array(K_old.sum(axis=1)).flatten()/((n-1)*h**d)
        q = q/m0
